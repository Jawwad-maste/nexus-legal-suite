...
import { Check, Star, ArrowLeft, Eye, EyeOff, Scale } from 'lucide-react';
...
const PricingPage = () => {
  ...
  const handlePlanSelect = async (planId: string) => {
    if (!user) {
      toast.error('You need to be logged in to select a plan. Please log in first.');
      return;
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {plans.map((plan, index) => (
              ...
              >
                ...
                <Card className={`h-full transition-all duration-200 hover:shadow-lg hover:scale-105 cursor-pointer hover:border-blue-400 ${
                  plan.highlighted 
